...
import { listLocalAdventures, deleteLocalAdventure, LocalAdventure } from "../misc/localAdventureManager";
...
export default function LocalAdventureList() {
  ...
  const handleDelete = async (e: React.MouseEvent, adventureId: string) => {
    e.stopPropagation();
    if (confirm("Are you sure you want to delete this local adventure? This cannot be undone.")) {
      try {
  ...
  if (adventures.length === 0) {
    return (
      <div className="text-center py-8 bg-gray-50 dark:bg-gray-800/50 rounded-xl border border-gray-200 dark:border-gray-700">
        <p className="text-gray-500 dark:text-gray-400">No locally saved adventures found.</p>
      </div>
    );
  }
        ...
        {adventures.map((adv) => (
          ...
          >
            <div className="flex justify-between items-start">
              <div className="flex-1 min-w-0 cursor-pointer" onClick={(e) => handlePlay(e, adv.id)}>
                <h4 className="font-bold text-gray-900 dark:text-white truncate pr-8">
                  {adv.title}
                </h4>
                <p className="text-sm text-gray-500 dark:text-gray-400 mt-1">
                  {new Date(adv.updatedAt).toLocaleDateString()} • {new Date(adv.updatedAt).toLocaleTimeString()}
                </p>
