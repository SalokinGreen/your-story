...
export default function LibraryPage() {
  ...
  const [storyFilter, setStoryFilter] = useState<"all" | "completed" | "inProgress">("all");
  const [storySortBy, setStorySortBy] = useState<StorySortBy>("updated");
  const [adventureSearch, setAdventureSearch] = useState("");
  const [adventureFilter, setAdventureFilter] = useState<"all" | "published" | "draft">("all");
  const [adventureSortBy, setAdventureSortBy] = useState<AdventureSortBy>("updated");
  ...
  const handleDeleteFolder = async (folderId: string) => {
    if (!confirm("Are you sure? Stories in this folder will not be deleted, just uncategorized.")) {
      return;
    }
    ...
    try {
      ...
      setStories(stories.map(s => s.folder_id === folderId ? { ...s, folder_id: null } : s));
  ...
  const handleMoveStory = async (storyId: string, folderId: string | null) => {
    try {
      ...
      setStories(stories.map(s => s.id === storyId ? { ...s, folder_id: folderId } : s));
      setMovingStory(null);
  ...
  const filteredStories = stories
    .filter(story => {
      ...
      // Search filter
      if (storySearch && !story.story_name.toLowerCase().includes(storySearch.toLowerCase())) {
        return false;
      }
      // Completion filter
  ...
  const filteredAdventures = adventures
    .filter(adventure => {
      // Search filter
      if (adventureSearch && 
          !adventure.title.toLowerCase().includes(adventureSearch.toLowerCase()) &&
          !adventure.description.toLowerCase().includes(adventureSearch.toLowerCase())) {
        return false;
      }
      // Published filter
      if (adventureFilter === "published" && !adventure.isPublished) return false;
    ...
    <div className="min-h-screen bg-linear-to-br from-blue-50 via-purple-50 to-pink-50 dark:from-gray-900 dark:via-purple-900 dark:to-blue-900">
      <div className="max-w-7xl mx-auto p-4 sm:p-8">
        ...
        <div className="flex gap-2 mb-8">
          ...
          >
            📖 My Stories ({filteredStories.length}{stories.length !== filteredStories.length ? ` / ${stories.length}` : ""})
          </button>
          <button
          ...
          >
            🎮 My Adventures ({filteredAdventures.length}{adventures.length !== filteredAdventures.length ? ` / ${adventures.length}` : ""})
          </button>
        </div>
        ...
        {loading ? (
                ...
                {folders.map(folder => (
                  <div key={folder.id} className="group relative">
                    ...
                    >
                      ...
                      {folder.name} ({stories.filter(s => s.folder_id === folder.id).length})
                    </button>
            ...
            <div className="bg-white dark:bg-gray-800 rounded-2xl shadow-xl p-6 mb-6 border border-gray-200 dark:border-gray-700">
              <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
                ...
                <div>
                  ...
                  <select
                    value={storyFilter}
                    onChange={(e) => setStoryFilter(e.target.value as "all" | "completed" | "inProgress")}
                    className="w-full px-4 py-2 bg-gray-50 dark:bg-gray-700 border-2 border-gray-300 dark:border-gray-600 rounded-lg focus:border-purple-500 dark:focus:border-purple-400 focus:outline-none text-gray-900 dark:text-white"
                  >
              ...
              {/* Clear Filters */}
              {(storySearch || storyFilter !== "all" || storySortBy !== "updated") && (
                    ...
                    <div className="flex items-start justify-between gap-4">
                      <div className="flex-1 min-w-0">
                        ...
                        <div className="flex flex-wrap gap-4 text-sm text-gray-600 dark:text-gray-400 mb-3">
                          ...
                          {story.story_data?.currentChapter !== undefined && (
                            <span>Chapter: {story.story_data.currentChapter + 1}</span>
                          )}
                        </div>
                      </div>
          ...
          <div>
            ...
            <div className="bg-white dark:bg-gray-800 rounded-2xl shadow-xl p-6 mb-6 border border-gray-200 dark:border-gray-700">
              <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
                ...
                <div>
                  ...
                  <select
                    ...
                    onChange={(e) => setAdventureFilter(e.target.value as "all" | "published" | "draft")}
                    className="w-full px-4 py-2 bg-gray-50 dark:bg-gray-700 border-2 border-gray-300 dark:border-gray-600 rounded-lg focus:border-purple-500 dark:focus:border-purple-400 focus:outline-none text-gray-900 dark:text-white"
                  >
                ...
                <div>
                  ...
                  <select
                    ...
                    onChange={(e) => setAdventureSortBy(e.target.value as AdventureSortBy)}
                    className="w-full px-4 py-2 bg-gray-50 dark:bg-gray-700 border-2 border-gray-300 dark:border-gray-600 rounded-lg focus:border-purple-500 dark:focus:border-purple-400 focus:outline-none text-gray-900 dark:text-white"
                  >
                    <option value="updated">Last Updated</option>
                    <option value="created">Date Created</option>
              ...
              {/* Clear Filters */}
              {(adventureSearch || adventureFilter !== "all" || adventureSortBy !== "updated") && (
                <div className="mt-4 flex justify-end">
                  <button
                    onClick={() => {
                      setAdventureSearch("");
            ...
            {filteredAdventures.length === 0 ? (
              <div className="bg-white dark:bg-gray-800 rounded-2xl shadow-xl p-12 text-center border border-gray-200 dark:border-gray-700">
                <div className="text-6xl mb-4">🎮</div>
                <h3 className="text-2xl font-bold mb-2 text-gray-900 dark:text-white">
                  {adventures.length === 0 ? "No Adventures Yet" : "No Adventures Match Filters"}
                </h3>
                <p className="text-gray-600 dark:text-gray-400 mb-6">
                  {adventures.length === 0 
                  ...
                  >
                    ...
                    <div className="p-4">
                      ...
                      <div className="flex gap-2 text-sm text-gray-600 dark:text-gray-400 mb-4">
                        <span>⭐ {adventure.rating?.toFixed(1) || "N/A"}</span>
                        <span>•</span>
                      ...
                      <div className="flex gap-2">
                        ...
                        <button
                          onClick={() => router.push(`/creator?edit=${adventure.id}`)}
        ...
        <div className="fixed inset-0 bg-black bg-opacity-50 flex items-center justify-center z-50 p-4">
          <div className="bg-white dark:bg-gray-800 rounded-lg p-6 max-w-md w-full">
            ...
            <div className="space-y-4">
              <div>
                ...
                <input
                  ...
                  value={newFolderName}
                  onChange={(e) => setNewFolderName(e.target.value)}
                  onKeyDown={(e) => e.key === "Enter" && editingFolder && handleUpdateFolder(editingFolder.id, { name: newFolderName, icon: newFolderIcon, color: newFolderColor })}
                  className="w-full px-3 py-2 bg-white dark:bg-gray-700 border border-gray-300 dark:border-gray-600 rounded-lg text-gray-900 dark:text-white"
                  placeholder="My Folder"
                  autoFocus
                />
            ...
            <div className="flex gap-3 mt-6">
              ...
              <button
                onClick={() => editingFolder && handleUpdateFolder(editingFolder.id, { name: newFolderName, icon: newFolderIcon, color: newFolderColor })}
                disabled={!newFolderName.trim()}
                className="flex-1 px-4 py-2 bg-purple-600 hover:bg-purple-700 text-white font-semibold rounded-lg transition-colors disabled:opacity-50 disabled:cursor-not-allowed"
              >
                Save
        ...
        <div className="fixed inset-0 bg-black bg-opacity-50 flex items-center justify-center z-50 p-4">
          <div className="bg-white dark:bg-gray-800 rounded-lg p-6 max-w-md w-full">
            ...
            <div className="space-y-2 max-h-96 overflow-y-auto">
              <button
                onClick={() => movingStory && handleMoveStory(movingStory, null)}
              ...
              {folders.map((folder) => (
                <button
                  key={folder.id}
                  onClick={() => movingStory && handleMoveStory(movingStory, folder.id)}
